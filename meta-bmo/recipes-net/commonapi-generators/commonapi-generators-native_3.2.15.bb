...
GENERATOR_DIR = "${datadir}/commonapi-generators"

do_install() {
	install -d ${D}${GENERATOR_DIR} ${D}${bindir}

	cp -rf ${S}/core   ${D}${GENERATOR_DIR}/
	cp -rf ${S}/someip ${D}${GENERATOR_DIR}/

	# The launchers are shipped without the execute bit in the zips.
	chmod +x ${D}${GENERATOR_DIR}/core/commonapi-core-generator-linux-x86_64
	chmod +x ${D}${GENERATOR_DIR}/someip/commonapi-someip-generator-linux-x86_64
